...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('🔄 Avvio sincronizzazione tab_viaggi (SOLO TERZISTI)...', isDryRun ? '(MODALITÀ DRY-RUN)' : '');
    if (days) {
    ...
    // Estrai dati da tab_vettori (database gestionelogistica) - SOLO TERZISTI
    console.log('🔄 Estrazione dati da tab_vettori (SOLO TERZISTI)...');
    ...
    console.log('📊 Trovati', vettoriData.length, 'record in tab_vettori (TERZISTI)');
    ...
    console.log('🔄 Elaborazione e inserimento dati TERZISTI (solo nuovi)...', isDryRun ? '(SIMULAZIONE)' : '');
    let insertedCount = 0;
    let skippedCount = 0;
    ...
    for (const viaggio of viaggiPodData) {
      ...
      if (vettore) { // Solo se è terzista (indipendentemente da travels)
        // Logica di fallback per vehicle: usa travel se disponibile, altrimenti usa vettore.Targa_Mezzo
        const vehicleId = travel ? travel.targaMezzoId : vettore.Targa_Mezzo;
        ...
        if (isDryRun) {
          ...
          if (checkResult[0].count === 0) {
            ...
            if (sampleNewRecords.length < 10) {
              sampleNewRecords.push({
                ...
                Targa: vehicleId || null
              });
            }
            console.log('🆕 [DRY-RUN] Nuovo record TERZISTA:', viaggio.Viaggio, '-', viaggio['Nome Trasportatore']);
          } else {
            skippedCount++;
            console.log('⏭️ [DRY-RUN] Record TERZISTA esistente:', viaggio.Viaggio, '-', viaggio['Nome Trasportatore']);
          }
        } else {
          ...
          // Logica di fallback per i campi principali
          const nominativo = travel?.nominativoId || (vettore.Cognome_Vettore && vettore.Nome_Vettore ? `${vettore.Cognome_Vettore} ${vettore.Nome_Vettore}` : null);
          const targa = travel?.targaMezzoId || vettore.Targa_Mezzo || null;
          const tipoPatente = vehicle?.tipo_patente || null;
    ...
    const logMessage = isDryRun 
      ? `✅ [DRY-RUN] Simulazione TERZISTI completata: ${insertedCount} righe verrebbero inserite, ${skippedCount} righe già esistenti`
      : `✅ Sincronizzazione TERZISTI completata: ${insertedCount} righe inserite, ${skippedCount} righe già esistenti`;
    ...
    const responseData: any = {
      ...
      daysFilter: days,
      message: isDryRun 
        ? `[DRY-RUN] Simulazione TERZISTI completata! ${insertedCount} righe verrebbero inserite, ${skippedCount} righe già esistenti${filterMessage}.`
        : `Sincronizzazione TERZISTI completata con successo! ${insertedCount} righe inserite, ${skippedCount} righe già esistenti${filterMessage}.`,
    ...
    return NextResponse.json({
      error: 'Errore durante la sincronizzazione TERZISTI',
