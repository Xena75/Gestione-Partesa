...
export default function PRDPage() {
  ...
  useEffect(() => {
    const loadDocument = async () => {
      try {
        const response = await fetch('/api/documents/prd');
        if (!response.ok) {
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <div className="flex items-center mb-2">
            <FileText size={24} className="text-blue-600 dark:text-blue-400 mr-2" />
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Product Requirements Document</h1>
          </div>
          <p className="text-gray-600 dark:text-gray-400">Documentazione dei requisiti del prodotto Gestione Partesa</p>
        </div>
