...
REM File di log
set LOG_FILE=%LOG_DIR%\backup-full_%timestamp%.log

echo [%date% %time%] Inizio backup completo >> "%LOG_FILE%"
echo [%date% %time%] Inizio backup completo
...
echo [%date% %time%] Backup viaggi_db... >> "%LOG_FILE%"
echo Backup viaggi_db...
...
if errorlevel 1 (
    echo [%date% %time%] ERRORE: Backup viaggi_db fallito >> "%LOG_FILE%"
    echo ERRORE: Backup viaggi_db fallito
    exit /b 1
)

REM Backup gestionelogistica
echo [%date% %time%] Backup gestionelogistica... >> "%LOG_FILE%"
echo Backup gestionelogistica...
...
if errorlevel 1 (
    echo [%date% %time%] ERRORE: Backup gestionelogistica fallito >> "%LOG_FILE%"
    echo ERRORE: Backup gestionelogistica fallito
    exit /b 1
)
...
REM Prima crea un job per il backup completo
"%MYSQL_BIN%\mysql.exe" -h %MYSQL_HOST% -P %MYSQL_PORT% -u %MYSQL_USER% -e "INSERT INTO backup_management.backup_jobs (backup_type, status, started_at) VALUES ('full', 'completed', NOW());" 2>> "%LOG_FILE%"
...
echo [%date% %time%] Backup completo completato con successo >> "%LOG_FILE%"
echo Backup completo completato con successo
echo File creati:
echo - %BACKUP_FILE_1%
echo - %BACKUP_FILE_2%
